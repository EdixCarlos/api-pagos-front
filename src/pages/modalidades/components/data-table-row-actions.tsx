...
import { EditModalidadDialog } from '@/pages/modalidades/components/EditModalidadDialog.tsx'
import { modalidadContext } from '@/context/modalidadesContext.tsx'
import { modalidadesSchema } from '@/domain/modalidadSchema.ts'
import { deleteModalidad } from '@/services/modalidadesService.ts'
interface IData {
  id: number;
  // otras propiedades...
}
...
export function DataTableRowActions<TData extends IData>({
  ...
  const modalidad = modalidadesSchema.parse(row.original)
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
  const handleDelete = async () => {
    try {
      await deleteModalidad(row.original.id);
      toast.toast({
        variant: 'success',
        title: 'Éxito',
        description: 'La modalidad se ha eliminado correctamente.',
      })
    } catch (error) {
      toast.toast({
        variant: 'error',
        title: 'Error',
        description: 'Hubo un error al eliminar la modalidad.',
      })
