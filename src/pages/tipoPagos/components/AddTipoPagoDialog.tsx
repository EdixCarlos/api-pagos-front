...
import { createTipoPago } from '@/services/tipoPagoService.ts'

export const AddTipoPagoDialog = ({ isOpen, setIsOpen }) => {
  const { fetchTipoPago } = useContext(tipoPagoContext);
  ...
  const handleAdd = async () => {
    try {
      const tipoPago = methods.getValues()
      const addTipoPago = {
        nombre: tipoPago.nombre,
      }
      await createTipoPago(addTipoPago)
      toast.toast({
        variant: 'success',
        title: 'Éxito',
        description: 'El tipo pago se ha añadido correctamente.'
      })
      fetchTipoPago()
      closeDialog()
      methods.reset()
      ...
      toast.toast({
        variant: 'error',
        title: 'Error',
        description: 'Hubo un error al añadir el tipo pago.'
      })
    }
    methods.reset()
  }
